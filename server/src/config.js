...
if (![undefined, 'development', 'production', 'test'].includes(process.env.NODE_ENV)) {
  console.error(
    `Fatal Error: NODE_ENV '${process.env.NODE_ENV}' not supported. Leave blank, or use development, production, demo, or test.`
  );
  process.exit(1);
}
...
const VALID_MIME_TYPES = ['image/png', 'image/jpeg', 'image/gif'];
